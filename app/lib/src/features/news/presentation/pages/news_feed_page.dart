...
// import 'package:cached_network_image/cached_network_image.dart'; // Unused
import '../providers/news_providers.dart';
import '../providers/filter_providers.dart';
import '../widgets/news_card.dart';
import '../widgets/news_filter_bar.dart';
import '../widgets/data_saver_image.dart';
...
class NewsFeedPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final newsAsync =
        ref.watch<AsyncValue<List<NewsArticle>>>(newsFeedProvider);
  ...
  static Widget _buildFallbackIcon(BuildContext context, NewsArticle article) {
    IconData icon = Icons.article;
    final color = Theme.of(context).colorScheme.primary.withValues(alpha: 0.1);
    final iconColor = Theme.of(context).colorScheme.primary;

    if (article.source.contains('Android')) {
      icon = Icons.android;
    } else if (article.source.contains('Firebase')) {
      icon = Icons.local_fire_department;
    } else if (article.source.contains('Flutter')) {
      icon = Icons.flutter_dash;
    } else if (article.source.contains('DeepMind') ||
        article.title.contains('AI')) {
      icon = Icons.psychology;
    } else if (article.source.contains('Chrome')) {
      icon = Icons.web;
    }
